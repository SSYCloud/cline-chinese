			...
			switch (type) {
				...
				case "command":
					return [
						...
						<span className="font-bold text-foreground">Cline 需要执行命令:</span>,
					]
				case "use_mcp_server":
				...
				case "followup":
					return [
						...
						<span className="font-bold text-foreground">Cline 有个问题:</span>,
					]
				default:
					return [null, null]
			}
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								{tool.operationIsLocatedInWorkspace === false &&
									toolIcon("sign-out", "yellow", -90, "此文件在工作目录之外")}
								<span className="font-bold">Cline 需要创建新文件:</span>
							</div>
							{backgroundEditEnabled && tool.path && tool.content ? (
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								{tool.operationIsLocatedInWorkspace === false &&
									toolIcon("sign-out", "yellow", -90, "此文件在工作目录之外")}
								<span className="font-bold">Cline 需要读取文件:</span>
							</div>
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								{toolIcon("search")}
								{tool.operationIsLocatedInWorkspace === false &&
									toolIcon("sign-out", "yellow", -90, "此目录在工作文件🏠之外")}
								<span className="font-bold">
									Cline 需要搜索文件夹 <code className="break-all">{tool.regex}</code>:
								</span>
							</div>
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								{tool.operationIsLocatedInWorkspace === false &&
									toolIcon("sign-out", "yellow", -90, "这是外部链接")}
								<span className="font-bold">
									{message.type === "ask" ? "Cline 需要从这个链接获取内容:" : "Cline 获取到内容:"}
								</span>
							</div>
							<div
						...
						{useMcpServer.type === "use_mcp_tool" && (
							<div>
								...
								{useMcpServer.arguments && useMcpServer.arguments !== "{}" && (
									<div className="mt-2">
										<div className="mb-1 opacity-80 uppercase">用户协议</div>
										<CodeAccordian
					...
					case "new_task":
						return (
							<div>
								<div className={HEADER_CLASSNAMES}>
									<FilePlus2Icon className="size-2" />
									<span className="text-foreground font-bold">Cline 需要开启新任务:</span>
								</div>
					...
					case "condense":
						return (
							<div>
								<div className={HEADER_CLASSNAMES}>
									<FilePlus2Icon className="size-2" />
									<span className="text-foreground font-bold">Cline 需要精简会话:</span>
								</div>
								<NewTaskPreview context={message.text || ""} />
							</div>
						)
					case "report_bug":
						return (
							<div>
								<div className={HEADER_CLASSNAMES}>
									<FilePlus2Icon className="size-2" />
									<span className="text-foreground font-bold">Cline 需要创建 Github issue:</span>
								</div>

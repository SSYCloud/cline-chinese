...
export class VsCodeLmHandler implements ApiHandler {
	...
	private async getClient(): Promise<vscode.LanguageModelChat> {
		...
		try {
			...
			throw new Error(`Cline Chinese <Language Model API>: Failed to select model: ${message}`)
		}
	}
	...
	async *createMessage(systemPrompt: string, messages: Message[], tools?: ToolDefinition[]): AsyncGenerator<ApiStreamChunk> {
		...
		try {
			const requestOptions: vscode.LanguageModelChatRequestOptions = {
				justification: `Cline 将使用 '${client.name}' 从 '${client.vendor}', 点击 '允许' 继续.`,
			}
			...
			if (error instanceof vscode.CancellationError) {
				throw new Error("Cline Chinese <Language Model API>: Request cancelled by user")
			}
			if (error instanceof Error) {
				Logger.error("Cline Chinese <Language Model API>: Stream error:", error)
				throw error
			}
			throw new Error(`Cline Chinese <Language Model API>: Response stream error: ${String(error)}`)
		} finally {
			this.ensureCleanState()
		}
	}

			...
			try {
				...
				if (
					...
					HostProvider.window.showMessage({
						...
						message: `无法初始化检查点管理器: ${errorMessage}`,
					});
				}
			}
		}
			...
			if (
				...
				if (isContextWindowExceededError) {
					...
					if (truncatedConversationHistory.length > 3) {
						ssyError.message = "超出上下文窗口限制。请点击“重试”以截断对话并再次尝试。";
				...
				const isSpendLimitError = ssyError.isErrorType(SSYErrorType.Balance)
				const quotaExceeded = ssyError.isErrorType(SSYErrorType.QuotaExceeded)
				const isEntitlementError = false
				...
				const isClineProviderInsufficientCredits = (() => {
					...
					try {
						const parsedError = SSYError.transform(
							error,
							model.id,
							providerId,
						);
						return parsedError.isErrorType(SSYErrorType.Balance);
					} catch {
						return false;
					}
				})();
		...
		try {
			...
			*/
			...
			try {
				...
				if (!this.taskState.abandoned) {
					const ssyError = ErrorService.get().toClineError(error, this.api.getModel().id)
					const errorMessage = ssyError.serialize()
					const isStreamingSpendLimitError = ssyError.isErrorType(SSYErrorType.Balance)

...
export class Controller {
	...
	constructor(readonly context: ClineExtensionContext) {
		...
		this.accountServiceSSY = new SSYAccountService(async () => {
			...
			Logger.log("[ Controller.accountServiceSSY ] :", apiConfiguration?.shengSuanYunToken)
	...
	*/
	...
	async handleOcaAuthCallback(code: string, state: string) {
		try {
			Logger.log("handleOcaAuthCallback:", code, state)
	...
	async getStateToPostToWebview(): Promise<ExtensionState> {
		...
		const welcomeViewCompleted = Boolean(
			this.stateManager.getGlobalStateKey("welcomeViewCompleted") || this.stateManager.getSecretKey("shengSuanYunToken"), //this.authService.getInfo()?.user?.uid,
		)

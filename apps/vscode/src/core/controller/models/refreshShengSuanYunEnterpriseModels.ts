...
export async function refreshShengSuanYunEnterpriseModels(
	...
	if (!token) {
		Logger.error(
			"No token found for ShengSuanYun. Cannot refresh enterprise models.",
		);
...
function buildEnterpriseModelInfo(
	rawProjects: Record<string, RawProject>,
): ShengSuanYunEnterpriseModelInfo {
	...
	for (const [idStr, project] of Object.entries(rawProjects)) {
		...
		for (const model of project.selectedModels ?? []) {
			if (
				EXCLUDE_KEYWORDS.some((kw) => model.name.includes(kw)) ||
				EXCLUDE_TYPES.some((tp) => model.type.includes(tp))
			) {
				continue;
			}
...
async function readCachedProjects(
	cacheFilePath: string,
): Promise<Record<string, RawProject> | undefined> {

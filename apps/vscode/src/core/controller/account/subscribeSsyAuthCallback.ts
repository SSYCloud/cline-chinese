...
export async function subscribeSsyAuthCallback(
	...
	if (requestId) {
		getRequestRegistry().registerRequest(
			requestId,
			cleanup,
			{ type: "authCallback_subscription" },
			responseStream,
		);
	}
}
 ...
 */
export async function sendSSYAuthCallbackEvent(
	customToken: string,
): Promise<void> {
	// Send the event to all active subscribers
	const promises = Array.from(activeAuthCallbackSubscriptions).map(

...
export async function initialize(storageContext: StorageContext): Promise<WebviewProvider> {
	...
	try {
		...
		Logger.error("[Cline Chinese] CRITICAL: Failed to initialize StateManager:", error)
...
async function showVersionUpdateAnnouncement(stateManager: StateManager) {
	...
	try {
		if (!previousVersion || currentVersion !== previousVersion) {
			Logger.log(`Cline Chinese version changed: ${previousVersion} -> ${currentVersion}. First run or update detected.`)
			...
			if (lastShownAnnouncementId !== latestAnnouncementId) {
				...
				const message = previousVersion
					? `Cline Chinese has been updated to v${currentVersion}`
					: `Welcome to Cline Chinese v${currentVersion}`
				HostProvider.window.showMessage({
...
export async function tearDown(): Promise<void> {
	try {
		...
		try {
			...
			Logger.error("[Cline Chinese] Failed to flush pending state during teardown:", error)
		}
	}
}

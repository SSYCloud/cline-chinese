...
3. Get access to the API with the following code:
    ...
    ```ts
    const clineExtension = vscode.extensions.getExtension<ClineAPI>("SSYCloud.cline-chinese")
    ...
    **Note:** To ensure that the `SSYCloud.cline-chinese` extension is activated before your extension, add it to the `extensionDependencies` in your `package.json`:

    ```json
    "extensionDependencies": [
        "SSYCloud.cline-chinese"
    ]
    ```

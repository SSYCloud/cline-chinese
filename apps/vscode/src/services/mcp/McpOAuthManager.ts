...
// key, in the format @coohu/core (CLI, JetBrains) reads and writes:
//
...
//  - Writes scope to ONE server's `oauth` key via @coohu/core's
...
//    @coohu/core's authorizeMcpServerOAuth, which binds a local loopback
 ...
 * providers. Matches @coohu/core's DEFAULT_HTTP_MCP_REDIRECT_URL — the actual
 ...
 * @coohu/core defaults; extras tolerate concurrent flows from other Cline
 ...
 * reads/writes the shared settings file in @coohu/core's format.
 *
 ...
 * HTTP-callback authorization flow via @coohu/core.
 */
export class McpOAuthManager {
	 ...
	 *
	 * Delegates to @coohu/core's authorizeMcpServerOAuth (the exact flow the

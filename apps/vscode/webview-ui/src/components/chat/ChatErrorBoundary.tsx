		...
		if (this.state.hasError) {
			return (
				<div
					...
					<h3 style={{ margin: "0 0 8px 0" }}>{errorTitle || "发生了错误"}</h3>
					<p style={{ margin: "0" }}>{errorBody || `Error: ${this.state.error?.message || "未知错误"}`}</p>
				</div>
			)
		}
	...
	render() {
		...
		return (
			<div
				...
				错误在第 {this.state.tickCount}/{this.props.numSecondsToWait ?? 5} 秒
			</div>
		)
	}
}

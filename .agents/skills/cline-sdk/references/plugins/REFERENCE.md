...
A Cline plugin is a TypeScript module that extends any agent built on the Cline SDK. The same plugin runs in the Cline CLI, VS Code and JetBrains extensions, and any custom app built on `@coohu/core`.
...
- `peerDependencies` for `@coohu/core` -- the host already provides it. Marking it optional lets you typecheck in isolation.

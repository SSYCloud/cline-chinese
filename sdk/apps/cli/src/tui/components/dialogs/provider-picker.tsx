...
export function UseExistingOrReconfigureContent(
	...
	return (
		<box flexDirection="column" gap={1}>
			<text>
				<strong>{providerName}</strong> 已经配置
			</text>
			...
			<text fg="gray">↑/↓ 导航，Enter 选中，Esc 返回</text>
		</box>
	);
}
...
export function CodexCliStatusContent(
	...
	return (
		<box flexDirection="column" paddingX={1} gap={1}>
			...
			{checking && <text fg="gray">检查 Codex CLI...</text>}
			...
			{status && !status.installed && (
				<box flexDirection="column" gap={1}>
					<text fg="yellow">Codex CLI 未找到</text>
...
export function OAuthLoginContent(
	...
	useEffect(() => {
		...
		loginLocalProvider(
			providerId as "shengsuanyun" | "oca" | "openai-codex",
			existing,
			(url: string) => {
				setAuthUrl(url);
				setStatus("等待浏览器认证...");
			...
			<box flexDirection="column" paddingX={1} gap={1}>
				...
				{deviceUserCode && !deviceError && (
					<box flexDirection="column" gap={1}>
						...
						<text fg="gray">访问此链接并输入上方的代码:</text>
						<text fg="cyan" selectable>
							{deviceVerifyUrl}
						</text>
					</box>

...
async function runHistoryUpdate(
	...
	if (!sessionId) {
		io.writeErr("历史更新需要 --session-id <id>");
		return 1;
	}
	...
	if (metadataStr) {
		try {
			...
			io.writeErr(
				`无效的元数据 JSON: ${error instanceof Error ? error.message : String(error)}`,
			);
			return 1;
		}
	}
	...
	if (prompt === undefined && metadata === undefined && title === undefined) {
		io.writeErr(
			"更新历史记录需要指定 --prompt <text>、--title <text> 或 --metadata <json>。",
		);
		return 1;
	}
...
async function runHistoryExport(
	...
	if (!sessionId) {
		io.writeErr("历史记录导出需要 <session-id>");
		return 1;
	}

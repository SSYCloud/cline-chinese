...
| Package | Owns |
|---------|------|
| `@coohu/shared` | Contracts, schemas, path helpers, hook engine, extension registry |
| `@coohu/llms` | Provider settings, model catalogs, manifests, handler creation |
| `@coohu/agents` | Stateless agent loop, tool orchestration, hook/extension runtime |
| `@coohu/core` | Stateful orchestration, session lifecycle, storage, config, telemetry, hub runtime services, hub discovery, detached daemon, and hub client adapters (`@coohu/core/hub`, `@coohu/core/hub/daemon-entry`) |
...
```sh
bun -F @coohu/core build|test|typecheck
bun -F @coohu/agents build|test|typecheck
```
...
The CLI build (`bun -F @coohu/cline build`) bundles packages from their compiled `dist/`, not their TypeScript source. If you edit a package and then build the CLI without rebuilding the package first, the CLI binary will silently include the old package code. Always run `bun run build:sdk` (or the relevant `bun -F @coohu/<pkg> build`) before building the CLI when testing changes end-to-end.
...
```sh
bun pm ls @coohu/core @coohu/agents @coohu/llms
```

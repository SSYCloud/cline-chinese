...
describe("display", () => {
	...
	describe("Spinner", () => {
		...
		it("should start spinning with message", () => {
			...
			expect(calls.some((c: any) => typeof c === "string" && c.includes("加载..."))).toBe(true)
		})
		...
		it("should update message", () => {
			...
			vi.advanceTimersByTime(80)
			const calls = writeSpy.mock.calls.map((c: any[]) => c[0])
			expect(calls.some((c: any) => typeof c === "string" && c.includes("更新"))).toBe(true)
		})
		...
		it("should stop with final message", () => {
			...
			spinner.stop("确定!")
			const calls = writeSpy.mock.calls.map((c: any[]) => c[0])
			expect(calls.some((c: any) => typeof c === "string" && c.includes("确定!"))).toBe(true)
		})

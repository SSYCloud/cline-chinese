...
program
	...
	.option("-a, --act", "运行执行模式")
	.option("-p, --plan", "运行规划模式")
	.option("-y, --yolo", "启用 yes/yolo 模式 (自动批准)")
	.option("-t, --timeout <seconds>", "可选超时时间（以秒为单位，仅在提供时生效）")
	.option("-m, --model <model>", "用于任务的模型")
	.option("-v, --verbose", "显示详细输出")
	.option("-c, --cwd <path>", "任务的工作目录")
	.option("--config <path>", "Cline 配置目录的路径")
	.option("--thinking [tokens]", "启用扩展思维（默认值：1024 个Token）")
	.option("--reasoning-effort <effort>", "推理强度: 无|低|中|高|最高")
	.option("--max-consecutive-mistakes <count>", "在 YOLO 模式下停止之前，连续犯错的最大次数")
	.option("--json", "以 JSON 格式输出消息，而不是以样式文本格式输出。")
	.option("--double-check-completion", "拒绝首次完成尝试以强制重新验证")
	.option("-T, --taskId <id>", "通过 ID 恢复现有任务")
...
program
	.command("history")
	.alias("h")
	.description("列出任务历史记录")
	.option("-n, --limit <number>", "要显示的任务数量", "10")
...
program
	.command("auth")
	.description("验证提供商身份并配置所使用的模型")
	.option("-p, --provider <id>", "用于快速设置的提供商 ID（例如，openai-native、anthropic、moonshot）")
	.option("-k, --apikey <key>", "提供商的 API 密钥")
	.option("-m, --modelid <id>", "要配置的模型 ID（例如，gpt-4o、claude-sonnet-4-6、kimi-k2.5）")
	.option("-b, --baseurl <url>", "基本 URL（可选，仅适用于 openai 提供商）")
	.option("-v, --verbose", "显示详细输出")
	.option("-c, --cwd <path>", "任务的工作目录")
	.option("--config <path>", "Cline 配置目录的路径")
	.action(runAuth)
...
program
	.command("update")
	.description("检查更新并安装（如有）。")
	.option("-v, --verbose", "显示详细输出")
	.action(() => checkForUpdates(CLI_VERSION))

// Dev command with subcommands
const devCommand = program.command("dev").description("开发者工具和实用程序")
...
program
	.argument("[prompt]", "任务提示（立即开始任务）")
	.option("-a, --act", "以活动模式运行")
	.option("-p, --plan", "以计划模式运行")
	.option("-y, --yolo", "启用 YOLO 模式（自动批准操作）")
	.option("-t, --timeout <seconds>", "可选超时时间（以秒为单位，仅在提供时生效）")
	.option("-m, --model <model>", "用于任务的模型")
	.option("-v, --verbose", "显示详细输出")
	.option("-c, --cwd <path>", "工作目录")
	.option("--config <path>", "配置目录")
	.option("--thinking [tokens]", "启用扩展思维（默认值：1024 Token）")
	.option("--reasoning-effort <effort>", "推理难度：无|低|中|高|极高")
	.option("--max-consecutive-mistakes <count>", "在 YOLO 模式下停止之前，连续犯错的最大次数")
	.option("--json", "以 JSON 格式输出消息，而不是以样式文本格式输出。")
	.option("--double-check-completion", "拒绝首次完成尝试以强制重新验证")
	.option("--acp", "以 ACP（代理客户端协议）模式运行，以便与编辑器集成")
	.option("-T, --taskId <id>", "通过 ID 恢复现有任务")

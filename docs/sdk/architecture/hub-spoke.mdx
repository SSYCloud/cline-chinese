...
<CardGroup cols={3}>
  ...
  <Card title="Spoke" icon="gear">
    Worker process running `@coohu/core`. Executes the agent loop, calls tools, streams output. Reports events back to the hub. Owned by the daemon, not by any client.
  </Card>

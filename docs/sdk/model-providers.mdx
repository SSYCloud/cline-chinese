...
The `@coohu/llms` package provides provider registries, model catalogs, handlers, and a gateway. `@coohu/agents` and `@coohu/core` use this layer internally.
...
```typescript
import { DefaultGateway } from "@coohu/llms"

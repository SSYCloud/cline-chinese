...
`@coohu/sdk` exports all SDK packages: `@coohu/core` for the full agent harness, `@coohu/agents` for the stateless agent loop, `@coohu/llms` for control over the model gateway, and `@coohu/shared` for common utilities.
...
| Package | Purpose |
|---------|---------|
| `@coohu/sdk` | Public SDK surface (re-exports `@coohu/core`) |
| `@coohu/core` | Node runtime for sessions, built-in tools, persistence, hub support, automation |
| `@coohu/agents` | Browser-compatible stateless agent execution loop |
| `@coohu/llms` | Provider gateway and model catalogs |
| `@coohu/shared` | Types, schemas, tool helpers, hooks, storage helpers |

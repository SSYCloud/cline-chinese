...
- `AgentRuntimeEvent` from `@coohu/agents`, delivered through `agent.subscribe(listener)`.
- `AgentEvent` / `CoreSessionEvent` from `@coohu/core`, delivered through core session adapters and `cline.subscribe(listener)`.
...
`AgentEvent` is the host-facing event shape used by `@coohu/core` session orchestration. For direct `Agent` usage, prefer `agent.subscribe(...)` and `AgentRuntimeEvent`.

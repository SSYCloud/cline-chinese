...
import { type AgentPlugin, createTool } from "@coohu/sdk"
...
Single-file plugins can only import Node builtins and `@coohu/*`. As soon as you need an npm dependency (`zod`, an HTTP client, etc.) you must ship as a package: a directory with a `package.json` that declares a `cline` field for entry points and your runtime `dependencies`. Dependencies under the `@coohu/` scope are provided by the host runtime -- the installer strips these and runs `npm install` for the rest, so declare any `@coohu/*` package you import as an optional peer dependency:

...
Dependencies under the `@coohu/` scope are provided by the host runtime. The installer automatically strips these from the plugin's dependency list before running `npm install`, so declare any `@coohu/*` package your plugin imports as an optional peer dependency. The host currently provides `@coohu/sdk`, `@coohu/core`, `@coohu/agents`, `@coohu/llms`, and `@coohu/shared`.

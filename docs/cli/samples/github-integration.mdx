...
description: "Automatically respond to GitHub issues by mentioning @coohu in comments using Cline CLI in GitHub Actions."
---

Automate GitHub issue analysis with AI. Mention `@coohu` in any issue comment to trigger an autonomous investigation that reads files, analyzes code, and provides actionable insights - all running automatically in GitHub Actions.
...
Trigger Cline by mentioning `@coohu` in any issue comment:

<Frame>
  <img src="https://storage.googleapis.com/cline_public_images/ss0a-comment.png" alt="Issue comment with @coohu mention" width="600" />
</Frame>
      ...
      - name: Check for @coohu mention
        ...
        with:
          script: |
            ...
            const hit = body.toLowerCase().includes("@coohu");
...
The workflow will look for new or updated issues, check for `@coohu` mentions, and then
...
Once set up, simply mention `@coohu` in any issue comment:

```text
@coohu what's causing this error?

@coohu analyze the root cause

@coohu what are the security implications?
```

GitHub Actions will:
1. Detect the `@coohu` mention
...
2. **Detects** `@coohu` mentions (case-insensitive)
